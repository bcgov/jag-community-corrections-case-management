...
export default {
  ...
  methods: {
    ...
    expandRow ({ item, value }) {
      ...
      this.getClientProfile(item.csNumber);
    },
    async getClientProfile(csNumber) {
      //console.log("Client profile search for csNumber: ", csNumber);
      const [error, response] = await clientProfileSearch(csNumber);
      if (error) {
        console.error(error);
      } else {
        ...
        if (this.initDataArray != null && this.initDataArray[csNumber] != null && this.initDataArray[csNumber].data != null) {
          this.initDataArray[csNumber].data.photo = "data:image/png;base64, " + response.photo.image;
          this.initDataArray[csNumber].data.datePhotoTaken = response.photo.photoTakenDate;
        }
        this.keyExpandRow++;
      }
      // TO BE REMOVED START
      ...
      //console.log("this.initDataArray[csNumber]: ", this.initDataArray, this.initDataArray[csNumber]);
      //Cache the photoData into this.initDataArray object
      if (  this.initDataArray != null && this.initDataArray[csNumber] != null 
         && this.initDataArray[csNumber].data != null) {
        this.initDataArray[csNumber].data.photo = "data:image/png;base64, " + sd;
        this.initDataArray[csNumber].data.datePhotoTaken = "2022-01-09";
      }
      this.keyExpandRow++;
      // TO BE REMOVED END
    },
